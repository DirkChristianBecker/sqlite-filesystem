...
static CREATE_SQL: &str = "CREATE TABLE x(path, is_dir, bytes, is_file, created, modified, accessed, is_sym_link, extension, file_name, input hidden)";
enum Columns {
    Path,
    IsDir,
    Bytes,
...
fn column(index: i32) -> Option<Columns> {
    match index {
        ...
        8 => Some(Columns::Extension),
        9 => Some(Columns::FileName),
        10 => Some(Columns::Input),
        _ => None,
    }
}
...
impl VTabCursor for ListDirectoryCursor {
    fn filter(
        ...
        for entry in paths {
            ...
            let (is_dir, size, is_file, is_sym_link, accessed, created, modified) =
